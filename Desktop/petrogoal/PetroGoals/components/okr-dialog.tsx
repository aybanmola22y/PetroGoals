...
import { OKR, KeyResult, Initiative, DEPARTMENTS, Department, TargetType, MilestoneStage, DEFAULT_MILESTONE_STAGES, StrategicPillar, User } from "@/types/okr"
import { COMPANY_INFO, store } from "@/lib/store"
...
export function OKRDialog({ open, onOpenChange, okr, onSubmit }: OKRDialogProps) {
  ...
  const [department, setDepartment] = React.useState<Department>(DEPARTMENTS[0])
  ...
  const isFormValid = () => {
    return (
      goal.trim() !== "" &&
      department.length > 0 &&
      keyResults.every(kr => kr.title.trim() !== "") &&
      initiatives.every(init => init.title.trim() !== "" && init.assignee !== "")
    )
  }
  ...
  const handleTargetTypeChange = (id: string, targetType: TargetType) => {
    ...
    if (kr && kr.targetType === targetType) return
    ...
    if (targetType === "milestone") {
      ...
      const initialStages = kr?.milestoneStages && kr.milestoneStages.length > 0
        ? kr.milestoneStages
        : createCustomMilestoneStages();
      const balancedStages = recalculateWeights(initialStages);

      updateKeyResult(id, {
        targetType,
        milestoneStages: balancedStages,
        target: 100,
        unit: "%"
      })
    } else {
  ...
  const addCustomStage = (krId: string) => {
    ...
    let newStages = [...kr.milestoneStages, {
      id: generateId(),
    ...
    newStages = recalculateWeights(newStages)

    const totalProgress = newStages.reduce((acc, stage) => {
      return acc + (stage.progress * stage.weight / 100)
    }, 0)

    updateKeyResult(krId, {
      milestoneStages: newStages,
  ...
  const removeCustomStage = (krId: string, stageId: string) => {
    const kr = keyResults.find(k => k.id === krId)
    if (!kr || !kr.milestoneStages || kr.milestoneStages.length <= 1) return

    let newStages = kr.milestoneStages.filter(s => s.id !== stageId)
      ...
      <DialogContent className="max-w-lg max-h-[90vh] w-[95vw] p-0 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-4">
                ...
                {initiatives.map((init, index) => (
                  <div key={init.id} className="p-4 border rounded-lg space-y-3 bg-muted/30">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">Initiative {index + 1}</span>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <Select
                          value={init.assignee || ""}
                          onValueChange={(v) => updateInitiative(init.id, { assignee: v })}
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          type="date"
                          value={init.deadline || ""}
                          onChange={(e) => updateInitiative(init.id, { deadline: e.target.value })}
                          className="h-9"
                        />
                      </div>
                    </div>
          ...
          <div className="px-6 py-4 border-t gap-3 flex justify-end">
            ...
            <Button type="submit" disabled={isLoading || !isFormValid()} data-testid="button-submit-okr">
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                  {okr ? "Updating..." : "Creating..."}
                </>
              ) : (
