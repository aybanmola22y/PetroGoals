...
export function StrategicPillarsOverview({ okrs }: StrategicPillarsOverviewProps) {
  ...
  const handleOKRClick = (okrId: string) => {
    router.push(`/dashboard/departments?okrId=${okrId}`)
  }
                ...
                okrs
                  ...
                  .map((okr) => {
                    ...
                    return (
                      <div 
                        key={okr.id} 
                        className="p-4 rounded-lg border bg-card hover:bg-muted/80 transition-all cursor-pointer group/okr relative"
                        onClick={() => handleOKRClick(okr.id)}
                      >
                        <div className="absolute top-4 right-4 opacity-0 group-hover/okr:opacity-100 transition-opacity">
                          <ExternalLink className="h-4 w-4 text-primary" />
                        </div>
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <div className="flex-1 min-w-0">
                            <h4 className="font-medium text-sm mb-1 group-hover/okr:text-primary transition-colors">{okr.goal}</h4>
